...
class FloatingButtonLayout():
    ...
    def __init__(self, floatingButtonText, actionButtons):
        ...
        self.floatingButton = FloatingButton(text=floatingButtonText)
        self.floatingButton.on_press = self.floatingButtonClick
...
class FeedFloatingButton(FloatingButtonLayout):
    def floatingButtonClick(self):
    ...
    def __init__(self, actionButtons):
        self.actionButtons = self.generateActionButtonsLayout(actionButtons)
        self.actionButtonsLayout = FloatLayout()
        self.floatingButton = FloatingButton(text="=")
        self.floatingButton.on_press = self.floatingButtonClick

class ProfileFloatingButton(FloatingButtonLayout):
    def floatingButtonClick(self):
        #subscribe
        pass
...
class Feed(Screen):
    def layout(self):
        ...
        floatingButton = FeedFloatingButton(actionButtons).layout()
    ...
    def layout(self):
        ...
        layout.add_widget(
                PostWithComments(username="username ", description="desc", size=(Window.width, Window.width * 9/4),
                     size_hint_y=None))
        mainWidget = ScrollView(size_hint=(1, None), size=(Window.width, Window.height))
...
class Profile(Screen):
    ...
    def layout(self):
        ...
        profileHeader = ProfileHeader(size=(Window.width, Window.width/3), size_hint=(None, None))

        galleryLayout = GridLayout(cols=3, spacing=0, size_hint_y   =None)
        galleryLayout.bind(minimum_height=galleryLayout.setter('height'))
        for i in range(100):
            galleryLayout.add_widget(Button(text='img', size_hint=(None, None), size=(Window.width/3,Window.width/3)))
        galleryRoot = ScrollView(size_hint=(1, None), size=(Window.width, Window.height - Window.width/3))
        ...
        actionButtons = []
        floatingButton = ProfileFloatingButton().layout()
        mainWidget.add_widget(floatingButton)
